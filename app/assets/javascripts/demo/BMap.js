/**
 * Created by maffee on 16/10/16.
 */
function focus_test_A(){

    var keyPoint = new BMap.Point(116.356951,39.986874);
    map.centerAndZoom(keyPoint,17);
}

function add_overlay_test_A_1(){

    var keyPoint = new BMap.Point(116.356951,39.986874);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.359772,39.98695),
        new BMap.Point(116.359862,39.985187),
        new BMap.Point(116.357055,39.985073)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red", fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    //map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_A_2(){
    var keyPoint = new BMap.Point(116.356102,39.990951);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.359588,39.991145),
        new BMap.Point(116.35974,39.988595),
        new BMap.Point(116.356093,39.988505)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red", fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    //map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_A_3(){
    var keyPoint = new BMap.Point(116.354674,39.986639);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.3567,39.9867),
        new BMap.Point(116.3568,39.985444),
        new BMap.Point(116.35568,39.985464),
        new BMap.Point(116.355599,39.985768),
        new BMap.Point(116.354719,39.985775)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    //map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_A_4(){
    var keyPoint = new BMap.Point(116.351642,39.986853);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.354616,39.98695),
        new BMap.Point(116.354759,39.985091),
        new BMap.Point(116.35175,39.984994)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    //map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_A_5(){
    var keyPoint = new BMap.Point(116.350802,39.991808);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.354404,39.991821),
        new BMap.Point(116.354503,39.990412),
        new BMap.Point(116.350802,39.990267)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    //map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_A_6(){
    var keyPoint = new BMap.Point(116.348197,39.984);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.351296,39.984193),
        new BMap.Point(116.352194,39.983674),
        new BMap.Point(116.352221,39.982251),
        new BMap.Point(116.350101,39.982195)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    //map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_B_1(){
    var keyPoint = new BMap.Point(116.464402,39.921089);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.465902,39.921085),
        new BMap.Point(116.465933,39.920328),
        new BMap.Point(116.464406,39.920328)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_B_2(){
    var keyPoint = new BMap.Point(116.358303,39.991833);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.359552,39.991888),
        new BMap.Point(116.359583,39.991162),
        new BMap.Point(116.358294,39.991149)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_B_3(){
    var keyPoint = new BMap.Point(116.359327,39.983627);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.360347,39.983599),
        new BMap.Point(116.360041,39.982487),
        new BMap.Point(116.359376,39.982466),
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_B_4(){
    var keyPoint = new BMap.Point(116.357144,39.983548);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(116.359322,39.9836),
        new BMap.Point(116.359322,39.982725),
        new BMap.Point(116.357238,39.982677)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_C_1(){
    var keyPoint = new BMap.Point(119.264317,31.565538);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(119.265583,31.5656),
        new BMap.Point(119.265462,31.564954),
        new BMap.Point(119.264559,31.564896)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    map.centerAndZoom(keyPoint,18);
}

function add_overlay_test_C_2(){
    var keyPoint = new BMap.Point(119.489593,31.399439);
    var polygon = new BMap.Polygon([
        keyPoint,
        new BMap.Point(119.493366,31.399254),
        new BMap.Point(119.493205,31.395632),
        new BMap.Point(119.489665,31.395848)
    ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

    map.clearOverlays();
    map.addOverlay(polygon);           //增加多边形覆盖物
    map.centerAndZoom(keyPoint,18);
}

function openDis(){
    var myDis = new BMapLib.DistanceTool(map);
    map.addEventListener("load",function(){
        myDis.open();  //开启鼠标测距
    });
}
function closeDis(){
    var myDis = new BMapLib.DistanceTool(map);
    map.addEventListener("load",function(){
        myDis.close();  //关闭鼠标测距
    });
}

function btn_nc(){

}

function btn_val(){

}

function btn_nvdi(){

}

function btn_other(){

}

function btn_ruler(){

}

function btn_draw(){

}

function btn_go(){
    var keyPoint = new BMap.Point(114.191877,23.130875);
    map.centerAndZoom(keyPoint,17);
}

function focus_boluo_A(){

    //kml <label>
    var ggn = 23.130875645889883;
    var ggs = 23.121166927809885;
    var gge = 114.20237471986587;
    var ggw = 114.19187761906588;

    //ABCD Point
    var wnP = new BMap.Point(ggw,ggn);
    var enP = new BMap.Point(gge,ggn);
    var esP = new BMap.Point(gge,ggs);
    var wsP = new BMap.Point(ggw,ggs);

    var points = [wnP,enP,esP,wsP];

    //array
    /*
    translateCallback = function (data){
        if(data.status === 0) {
            for (var i = 0; i < data.points.length; i++) {
                map.addOverlay(new BMap.Marker(data.points[i]));
            }
        }
    }*/
    //4
    translateCallback = function (data){
            if(data.status === 0) {
                var BwnP = data.points[0];
                var BenP = data.points[1];
                var BesP = data.points[2];
                var BwsP = data.points[3];

                //map.addOverlay(new BMap.Marker(BwnP));
                //map.addOverlay(new BMap.Marker(BenP));
                //map.addOverlay(new BMap.Marker(BesP));
                //map.addOverlay(new BMap.Marker(BwsP));

                var polygon = new BMap.Polygon([
                    BwnP,BenP,BesP,BwsP
                    //wnP,enP,esP,wsP
                ], {strokeColor:"lightblue", strokeStyle:"dashed" ,strokeWeight:2, strokeOpacity:0.8, fillOpacity:0.4, fillColor:"red"});  //创建多边形

                //map.clearOverlays();
                map.addOverlay(polygon);           //增加多边形覆盖物
            }
        };

    var convertor = new BMap.Convertor();
    convertor.translate(points, 1, 5, translateCallback);


}